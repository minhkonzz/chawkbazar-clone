...
export default forwardRef(function Cart({ onClose }: Props, ref: ForwardedRef<HTMLDivElement | null>) {
   ...
   return (
      <div {...{ref}} className={`${styles.container} d-flex fd-col jc-sb posab right-0 top-0 bottom-0`}>
         <header className={`${styles.header} d-flex jc-sb at-center`}>
            <h2 className={`${styles.heading} fw-700`}>Shopping cart</h2>
            <button className={`${styles.closeButton} d-flex jc-center at-center cp`} aria-label="close" onClick={e => onClose(e, true)}>
               <svg 
                  stroke="currentColor" 
                  fill="currentColor" 
                  strokeWidth="0" 
