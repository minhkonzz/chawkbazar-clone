...
const cartReducer = createSlice({
   ...
   reducers: {
      ...
      adjustAmount: (state, action) => {
         ...
         const { price: unitPrice, sale_price, sizeSelected, colorSelected } = itemAdjust; 
         const currentItems = state.cartItems; 
         ...
         state.cartTotalPrice = state.cartTotalPrice + (adjustType === MeanVars.INCREASE_ONCE ? priceChange : -priceChange) + (+0);
      }
   }
})
