      ...
      <div className="wrapper1920">
        <div className={styles.wrapper}>
          <div className={`${styles.message} d-flex at-center`}>
            <span
              className={`${styles.confirmIcWrapper} d-flex at-center jc-center`}>
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth="0"
          ...
          <div className={styles.detail}>
            ...
            <table className={`${styles.ordered} w-100pc`}>
              ...
              <tbody>
                {order?.products.map((e: CartItem, i: number) => (
                  <tr key={`${e.id}-${i}`} className={styles.row}>
                    ...
                    <td
                      className={
                        styles.cell
                      }>{`$${fixDecimal(e.lastPrice * e.qty, 2)}`}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                {[
                  {
                    id: "t1",
                    title: "Subtotal",
                    value: `$${fixDecimal(order.subtotal, 2)}`
                  },
                  {
                    id: "t2",
                    title: "Shipping",
                    value: `$${fixDecimal(order.shipFee, 2)}`
                  },
                  { id: "t3", title: "Tax", value: "$0.00" },
                  {
                    id: "t4",
                    title: "Total",
                    value: `$${fixDecimal(order.total, 2)}`
                  },
                  ...(order.note
